...
If you want to use a different color scheme, the `col` parameter allows you modify this (actually belongs to the `image` function) . By default the color scheme is set to `heat.colors(12)`:

```{r heatmap_cols}
heat.colors(12)
```

This basically gives us a color palette that can be correlated to the values in a matrix. The `RColorBrewer` R package provides a way to make your own colour palette:
...
Here we are setting a color gradient such as it goes from green -> black -> red which corresponds to low -> mid -> high values. You can play around with this to get the exact color scheme you want. 
