...
That looks really funny hey? What is actually happening? We see a strong "striping" pattern. Let's take a look at expression data we are working with. 
...
What we really want to do is to put every row onto the same scale. Right now the range of each row is a function of its expression level. What we can do is perform what is called Z-score normalization. Specifically:

$$ z = \frac{x - \mu}{\sigma} $$

Where by each value in a row, $x$, is subtracted by the row mean, $\mu$, and then divided by standard deviation $\sigma$. This effectively converts each value in a z-score and also places all rows onto the same scale. The `d3heatmap` function provides the ability to do a z-score row normalization by using the `scale = "row"`:
...
This looks a bit better. We no longer have the "stripe" patterns anymore. 
...
The `d3heatmap` function (and `heatmap`) actually performs the clustering automatically for you. In the previous section, I had turned it off using `Rowv = NA` and `Colv = NA` demonstrates only the heatmap component of the function. Let's see what happens if we enable clustering following normalization.

```{r}
d3heatmap(mat.lim, scale = "row", colors = rev(brewer.pal(9, "RdBu")), 
          width = 750, height = 1000, yaxis_width = 180)
```
